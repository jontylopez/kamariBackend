...
POS_ORDER_ITEM.belongsTo(INVENTORY, {
  foreignKey: "inventory_id",
  as: "inventory",
});
POS_ORDER_ITEM.belongsTo(STOCK_MOVEMENT, {
...
POS_RETURN_ITEM.belongsTo(INVENTORY, {
  foreignKey: "inventory_id",
  as: "inventory",
});
...
POS_ORDER.hasMany(POS_PAYMENT, { foreignKey: "order_id", as: "payments" });
...
const generateCSVReportForSession = async (sessionId) => {
  ...
  try {
    ...
    csvData.push(
      `${orders.length},${itemsSold},${totalRevenue.toFixed(
        2
  ...
  try {
    ...
    if (orders.length) {
      orders.forEach((order) => {
        csvData.push(
          `${order.id},${order.customer_id || "Guest"},${order.order_date},${
            order.order_time
  ...
  try {
      ...
      where: {
        order_id: {
          [Op.in]: Sequelize.literal(
            `(SELECT id FROM pos_order WHERE session_id = ${sessionId})`
          ),
        },
      },
    });
    ...
    csvData.push(
      "Inventory ID,Barcode,Sell Price,Buy Price,Quantity,Discount,Supplier"
    );

    Object.values(grouped).forEach((b) => {
      csvData.push(
        `${b.inventory_id},${b.b_code_id},${b.sell_price},${b.buy_price},${
          b.quantity
        },${b.discount || 0},${b.supplier}`
      );
    });
  ...
  try {
    const cashMovements = await CASH_TRANSACTION.findAll({
      where: { session_id: sessionId },
    });
    ...
    if (cashMovements.length) {
      cashMovements.forEach((entry) => {
        csvData.push(
          `${entry.type},${entry.reason || "-"},${entry.amount},${entry.date},${
            entry.time
          }`
        );
      });
  ...
  try {
    ...
    const itemDiscounts = orders.flatMap((o) =>
      o.items.filter((i) => i.discount > 0)
    );
...
const generateCSVReportForDatePeriod = async (startDate, endDate) => {
  const csvData = [];
  ...
  try {
    ...
    csvData.push("Daily Sales Summary Report");
    csvData.push("Date,Total Orders,Items Sold,Total Revenue,Total Discount");
    csvData.push(
      `${startDate} to ${endDate},${
        orders.length
      },${itemsSold},${totalRevenue.toFixed(2)},${totalDiscount.toFixed(2)}`
    );
    csvData.push("");
  } catch (err) {
    csvData.push("Daily Sales Summary Report");
  ...
  try {
    const orderItems = await POS_ORDER_ITEM.findAll({
      ...
      where: {
        order_id: {
          [Op.in]: Sequelize.literal(
            `(SELECT id FROM pos_order WHERE order_date BETWEEN '${startDate}' AND '${endDate}')`
          ),
        },
      },
    });

    // Get the order dates in a separate query
    const orderDates = await POS_ORDER.findAll({
    ...
    orderDates.forEach((order) => {
      orderDateMap[order.id] = order.order_date;
    });
    ...
    orderItems.forEach((item) => {
      ...
      const key = `${item.inventory_id}-${sellPrice}-${buyPrice}-${orderDate}`;
    ...
    csvData.push("Sold Items Breakdown");
    csvData.push(
      "Date,Inventory ID,Barcode,Sell Price,Buy Price,Quantity,Discount,Supplier"
    );
    ...
    Object.values(grouped)
      ...
      .forEach((b) => {
        csvData.push(
          `${b.date},${b.inventory_id},${b.b_code_id},${b.sell_price},${
            b.buy_price
          },${b.quantity},${b.discount || 0},${b.supplier}`
        );
      });
    ...
    console.error("Error in Sold Items Breakdown:", err);
    csvData.push("Sold Items Breakdown");
    csvData.push("Error generating section");
    csvData.push("");
  }
  ...
  try {
    ...
    csvData.push("Cash Movement Report");
    csvData.push("Date,Type,Reason,Amount,Time");

    if (cashMovements.length) {
      cashMovements.forEach((entry) => {
        csvData.push(
          `${entry.date},${entry.type},${entry.reason || "-"},${entry.amount},${
            entry.time
          }`
        );
      });
    ...
    console.error("Error in Cash Movement Report:", err);
    csvData.push("Cash Movement Report");
    csvData.push("Error generating section");
    csvData.push("");
  }
  ...
  try {
    ...
    orders.forEach((order) => {
      ...
      order.payments.forEach((payment) => {
        if (!totals[date][payment.method]) {
          totals[date][payment.method] = 0;
        }
        totals[date][payment.method] += parseFloat(payment.amount);
      });
    });

    csvData.push("Payment Method Summary");
    csvData.push("Date,Method,Amount");

    if (Object.keys(totals).length) {
      Object.entries(totals)
        .sort(([dateA], [dateB]) => new Date(dateA) - new Date(dateB))
        .forEach(([date, methods]) => {
          Object.entries(methods)
            .sort(([methodA], [methodB]) => methodA.localeCompare(methodB))
            .forEach(([method, amount]) => {
              csvData.push(`${date},${method},${amount.toFixed(2)}`);
            });
        });
    } else {
      csvData.push("No Data,No Data,0.00");
    }

    csvData.push("");
  } catch (err) {
    console.error("Error in Payment Method Summary:", err);
    csvData.push("Payment Method Summary");
    csvData.push("Error generating section");
    csvData.push("");
  }
  ...
  try {
    ...
    csvData.push("Discount Report");
    csvData.push("Date,Order ID,Order Discount,Item ID,Item Discount");

    const orderDiscounts = orders.filter((o) => o.discount > 0);
    const itemDiscounts = orders.flatMap((o) =>
      o.items
        .filter((i) => i.discount > 0)
        .map((i) => ({ ...i.dataValues, order_date: o.order_date }))
    );

    orderDiscounts.forEach((o) => {
      csvData.push(`${o.order_date},${o.id},${o.discount},,`);
    });

    itemDiscounts.forEach((i) => {
      csvData.push(`${i.order_date},,${i.inventory_id},${i.discount}`);
    });
    ...
    console.error("Error in Discount Report:", err);
    csvData.push("Discount Report");
    csvData.push("Error generating section");
    csvData.push("");
  }
  ...
  try {
    ...
    console.error("Error in Returns Report:", err);
    csvData.push("Returns Report");
    csvData.push("Error generating section");
    csvData.push("");
  }
