...
const generateBarcodePrefix = async (product_id) => {
    ...
    const genderCode = product.gender === 'female' ? 'W' : product.gender === 'male' ? 'M' : 'U';
    const typeMap = {
      UpperBody: 'UP',
      LowerBody: 'LO',
      FullBody: 'FU',
  ...
  const isDuplicateInventory = async ({ name, product_id, sup_br_id, size, color, description }) => {
  ...
  const createInventoryWithBarcode = async (data) => {
    try {
      const {
        name, product_id, sup_br_id,
        size, color, description
      } = data;
  
      const isDuplicate = await isDuplicateInventory({ name, product_id, sup_br_id, size, color, description });
